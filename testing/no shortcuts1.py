...
seed_results = {name: {"full": [], "no_port": [], "no_shortcut": []} for name in make_models().keys()}
...
for seed in SEEDS:
    ...
    Xns_train, Xns_test, _, _ = train_test_split(
        X_no_shortcut, y, test_size=0.20, random_state=seed, stratify=y
    )
    ...
    for name in make_models().keys():
        ...
        model_ns = make_models()[name]

        model_ns.fit(Xns_train, y_train_s)

        f1_no_shortcut = f1_score(
            y_test_s,
            model_ns.predict(Xns_test),
            average="weighted",
            zero_division=0
        )

        seed_results[name]["no_shortcut"].append(f1_no_shortcut)
        
        seed_results[name]["full"].append(f1_full)
        seed_results[name]["no_port"].append(f1_no_port)

print(f"\n{'Model':<22}{'Full':>10}{'NoPort':>12}{'NoShortcut':>15}")
step6_rows = []
for name, s in seed_results.items():
    full_arr = np.array(s["full"])
    no_port_arr = np.array(s["no_port"])
    no_shortcut_arr = np.array(s["no_shortcut"])
    try:
        t_stat, p_val = ttest_rel(full_arr, no_port_arr)
    except Exception:
        p_val = float("nan")
    print(f"{name:<22}{full_arr.mean():>10.4f}{no_port_arr.mean():>12.4f}{no_shortcut_arr.mean():>15.4f}")
    step6_rows.append({
        "Model": name,
        "Mean F1 (Full)": full_arr.mean(),
        "Mean F1 (No Port)": no_port_arr.mean(),
        "Mean F1 (No Shortcut)": no_shortcut_arr.mean(),
        "Std F1 (Full)": full_arr.std(),
        "Std F1 (No Port)": no_port_arr.std(),
        "Std F1 (No Shortcut)": no_shortcut_arr.std(),
        "p-value": p_val,
    })
